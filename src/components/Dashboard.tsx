...
import { db, storage, firebaseConfig, auth } from '../firebase';
...
import { UserProfile, ProductRequest, Offer, Category, Chat, MarketTrend, PriceInsight, AppFeatures, ContactEvent } from '../types';
import { QRCodeCanvas } from 'qrcode.react';
import { motion, AnimatePresence } from 'motion/react';
import SupplierProfileModal from './UserProfileModal';
import { DeleteAllCategoriesModal } from './DeleteAllCategoriesModal';
...
import { isAdmin, isSupplier, isCustomer } from '../utils/rbac';
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="text-[10px] font-bold text-brand-primary uppercase tracking-widest hover:underline disabled:opacity-50 flex items-center gap-1"
              >
                {isExtracting ? (
              ...
              {suggestedKeywords.map(kw => (
                <div key={kw} className="flex items-center gap-1 bg-brand-surface border border-brand-border rounded-xl overflow-hidden shadow-sm">
                  ...
                  <button 
                    ...
                    className="p-1.5 text-brand-success hover:bg-brand-success/10 transition-colors"
                  ...
                  <button 
                    ...
                    className="p-1.5 text-brand-error hover:bg-brand-error/10 transition-colors"
                    title="Dismiss"
                  >
                    <X size={14} />
                  </button>
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-brand-surface rounded-[2rem] border border-brand-border overflow-hidden shadow-sm hover:shadow-md transition-all group">
      <div className="bg-brand-background/50 px-6 py-5 flex justify-between items-center border-b border-brand-border">
        <div className="flex items-center gap-4">
          <div {...attributes} {...listeners} className="cursor-grab text-brand-text-muted hover:text-brand-primary p-1 hover:bg-brand-surface rounded-lg transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="text-brand-text-muted hover:text-brand-primary p-2 rounded-xl hover:bg-brand-primary/10 transition-all opacity-0 group-hover:opacity-100 flex items-center gap-1.5 relative"
          ...
          {confirmDeleteId === mainCat.id ? (
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="text-brand-text-muted hover:text-brand-text-main p-1.5 bg-brand-surface rounded-lg border border-brand-border"
              >
                <X size={14} />
              </button>
            </div>
...
const Dashboard: React.FC<DashboardProps> = ({ 
    ...
    return (
      <div ref={setNodeRef} style={style} className="relative group">
        <div 
          {...attributes} 
          {...listeners}
          className={`absolute top-2 ${isRtl ? 'left-2' : 'right-2'} p-1.5 cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity z-20 bg-white/90 backdrop-blur-sm rounded-lg border border-brand-border-light shadow-sm`}
  ...
  const [adminTab, setAdminTab] = useState<'overview' | 'suppliers' | 'users' | 'categories' | 'chats' | 'moderation' | 'settings' | 'market-trends' | 'price-intelligence' | 'user-data' | 'marketing'>('overview');
  ...
  const handleSuggestMainCategories = async () => {
    ...
    try {
      ...
      const suggestions = await suggestMainCategories(i18n.language, existingNames);
  ...
  const handleSuggestSubcategories = async () => {
    ...
    try {
      const existingSubNames = categories
        ...
        .map(c => i18n.language === 'ar' ? c.nameAr : c.nameEn);
      const suggestions = await suggestSubcategories(categoryName, existingSubNames);
      setAiSuggestions(suggestions);
    } catch (error) {
      console.error('Suggestion error:', error);
    } finally {
  ...
  const mainCategories = categories.filter(c => !c.parentId).sort((a, b) => (a.order || 0) - (b.order || 0));
    ...
    return (
      <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 text-brand-primary hover:text-brand-primary-hover disabled:opacity-50 p-2 hover:bg-brand-primary/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 focus:opacity-100"
                    ...
                    <div className="flex flex-wrap gap-2">
                      {newSuppKeywords.map((kw, index) => (
                        <span key={`${kw}-${index}`} className="px-2 py-1 bg-brand-primary/10 text-brand-primary-hover rounded-lg text-[10px] font-bold border border-brand-primary/20 flex items-center gap-1">
                          {kw}
                          <button type="button" onClick={() => setNewSuppKeywords(newSuppKeywords.filter(k => k !== kw))} className="text-brand-primary hover:text-brand-primary">
                            <X size={12} />
                          </button>
                        </span>
                      ))}
                  ...
                  <div className="md:col-span-2 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {editSuppKeywords.map((kw, index) => (
                        <span key={`${kw}-${index}`} className="px-2 py-1 bg-brand-primary/10 text-brand-primary-hover rounded-lg text-[10px] font-bold border border-brand-primary/20 flex items-center gap-1">
                          {kw}
                          <button type="button" onClick={() => setEditSuppKeywords(editSuppKeywords.filter(k => k !== kw))} className="text-brand-primary hover:text-brand-primary">
                            <X size={12} />
                          </button>
                        </span>
                      ))}
        ...
        <section className="h-[calc(100vh-140px)] min-h-[600px] flex flex-col lg:flex-row gap-6">
          {/* Left Pane: List */}
          <div className={`w-full lg:w-1/3 flex flex-col bg-brand-surface rounded-[2rem] border border-brand-border shadow-sm overflow-hidden h-full ${selectedAdminUser || isAddingUser ? 'hidden lg:flex' : 'flex'}`}>
            <div className="p-6 border-b border-brand-border bg-brand-background/50 flex flex-col gap-4 shrink-0">
              <div className="flex items-center justify-between">
  ...
  const renderSupplier = () => (
    <div className="p-6 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-brand-surface p-6 rounded-3xl border border-brand-border shadow-sm mb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-brand-primary/10 rounded-2xl flex items-center justify-center text-brand-primary">
            {supplierTab === 'dashboard' ? <LayoutGrid size={24} /> : 
             supplierTab === 'personal' ? <UserIcon size={24} /> : 
             supplierTab === 'marketing' ? <Megaphone size={24} /> : <Building2 size={24} />}
          </div>
          <div>
            <h1 className="text-2xl font-black text-brand-text-main tracking-tight">
                ...
                <div className="mt-10 pt-8 border-t border-brand-border">
                  <div className="mb-8">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs font-bold text-brand-primary hover:text-brand-primary-hover bg-brand-primary/10 hover:bg-brand-primary/20 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 mt-2"
                    >
                  ...
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const { generateKeywords } = await import('../services/geminiService');
                          const selectedCategoryNames = categories
                            .filter(c => editCategories.includes(c.id))
                            .map(c => i18n.language === 'ar' ? c.nameAr : c.nameEn);
      ...
      {supplierTab === 'dashboard' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-brand-surface p-6 rounded-[2rem] border border-brand-border shadow-xl shadow-brand-primary/5 flex items-center gap-4">
              ...
              <div>
                ...
                <h4 className="text-2xl font-black text-brand-text-main">{allChats.length}</h4>
              </div>
            </div>
            <div className="bg-brand-surface p-6 rounded-[2rem] border border-brand-border shadow-xl shadow-brand-primary/5 flex items-center gap-4">
              <div className="p-4 bg-brand-primary/10 rounded-2xl text-brand-primary">
              ...
              <div>
                ...
                <h4 className="text-2xl font-black text-brand-text-main">
            ...
            <div className="bg-brand-surface p-6 rounded-[2rem] border border-brand-border shadow-xl shadow-brand-primary/5 flex items-center gap-4">
              ...
              <div>
                ...
                <h4 className="text-2xl font-black text-brand-text-main">{profile?.rating?.toFixed(1) || '0.0'}</h4>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div className="space-y-1">
              <h3 className="text-3xl font-black text-brand-text-main flex items-center gap-3 tracking-tight">
                <div className="p-2.5 bg-brand-primary rounded-2xl text-white shadow-lg shadow-brand-primary/20">
                  <Sparkles size={24} />
                </div>
                {t('available_requests')}
              </h3>
              <p className="text-brand-text-muted text-sm font-medium ml-14">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <div className="relative flex-1 md:w-80 group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-text-muted group-focus-within:text-brand-primary transition-colors" size={20} />
                <input 
                  type="text" 
                  value={requestSearch}
                  onChange={e => {
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-brand-surface border border-brand-border rounded-[1.25rem] outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary transition-all text-sm font-medium shadow-sm text-brand-text-main"
                />
              </div>
              <button
                onClick={handleSemanticSearch}
                disabled={isSemanticSearching || !requestSearch}
                className="p-3.5 bg-brand-primary text-white rounded-[1.25rem] hover:bg-brand-primary-hover transition-all disabled:opacity-50 shadow-lg shadow-brand-primary/20 flex items-center justify-center group"
                title={i18n.language === 'ar' ? 'بحث ذكي بالذكاء الاصطناعي' : 'Smart AI Search'}
              >
                {isSemanticSearching ? (
                  ...
                  .map(req => {
                    ...
                    return (
                      <motion.div 
                        ...
                        exit={{ opacity: 0, scale: 0.95 }}
                        layout
                        className="bg-brand-surface p-8 rounded-[2.5rem] border border-brand-border shadow-xl shadow-brand-primary/5 hover:shadow-2xl hover:shadow-brand-primary/10 transition-all duration-500 group"
                      >
                        <div className="flex flex-col lg:flex-row gap-8">
                          <div className="flex-1 space-y-4">
                            <div className="flex justify-between items-start">
                              <div className="space-y-2">
                                <div className="flex items-center gap-3">
                                  ...
                                  {isNew && (
                                    <span className="px-2.5 py-1 bg-brand-primary text-white text-[10px] font-black rounded-full uppercase tracking-widest animate-pulse">
                                      {i18n.language === 'ar' ? 'جديد' : 'New'}
                                    </span>
                                  )}
                                </div>
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className="px-3 py-1 bg-brand-primary/10 text-brand-primary text-[11px] font-bold rounded-xl uppercase tracking-wider border border-brand-primary/20">
                                    {getCategoryPath(req.categoryId)}
                                  </span>
                                  <span className="flex items-center gap-1.5 text-brand-text-muted text-xs font-bold bg-brand-background px-3 py-1 rounded-xl">
                            ...
                            <div className="flex items-center gap-4 py-2">
                              <div className="flex items-center gap-2 text-xs font-bold text-brand-text-muted bg-brand-background/50 px-4 py-2 rounded-xl border border-brand-border/50">
...
const SupplierOfferAction: React.FC<{ 
  ...
  const handleGenerateAiMessage = async () => {
    ...
    try {
      const { generateSupplierProposal } = await import('../services/geminiService');
