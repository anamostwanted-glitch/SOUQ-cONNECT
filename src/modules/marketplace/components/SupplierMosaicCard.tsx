...
export const SupplierMosaicCard: React.FC<SupplierMosaicCardProps> = ({ 
  ...
  return (
    <HolographicHUD
      metadata={{
        id: supplier.uid.substring(0, 8),
        type: 'SUPPLIER_NODE',
        status: supplier.isOnline ? 'LIVE' : 'IDLE',
        origin: 'TRUST_NETWORK'
      }}
    >
      <motion.div
      ...
      >
        ...
        <div className="relative z-10 mt-auto pt-2 flex gap-2">
          ...
          <MagneticWrapper strength={10} className="flex-1">
            <HapticButton className="w-full py-2 bg-brand-primary/10 text-brand-primary rounded-xl text-[9px] font-black uppercase tracking-widest group-hover:bg-brand-primary group-hover:text-white group-hover:shadow-md transition-all flex items-center justify-center gap-1.5">
              {isRtl ? 'زيارة' : 'Visit'}
