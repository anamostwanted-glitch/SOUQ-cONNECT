...
export const BroadcastBox: React.FC<BroadcastBoxProps> = ({ t, i18n, allUsers, size = 'default' }) => {
  ...
  const handleSend = async () => {
    if (!message.trim() || !title.trim()) {
      toast.error(i18n.language === 'ar' ? 'يرجى إدخال العنوان والرسالة' : 'Please enter title and message');
      return;
    }
    setIsSending(true);
    try {
  ...
  return (
    <div className={`bg-white ${size === 'compact' ? 'p-4' : 'p-6'} rounded-[2rem] border border-brand-border-light shadow-sm`}>
      ...
      <input 
        ...
        className={`w-full px-4 ${size === 'compact' ? 'py-2' : 'py-2.5'} bg-brand-surface border border-brand-border rounded-xl mb-3 text-sm outline-none focus:border-brand-primary`}
      />
      <textarea 
        value={message}
        onChange={e => setMessage(e.target.value)}
        placeholder={i18n.language === 'ar' ? 'الرسالة' : 'Message'}
        className={`w-full px-4 ${size === 'compact' ? 'py-2' : 'py-2.5'} bg-brand-surface border border-brand-border rounded-xl mb-3 text-sm outline-none focus:border-brand-primary resize-none`}
