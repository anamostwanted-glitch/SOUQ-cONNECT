...
import { smartCache } from '../../../core/utils/smartCache';
import { Type } from '@google/genai';

interface UserInsightsHubProps {
  profile: UserProfile;
  isRtl: boolean;
}

export const UserInsightsHub: React.FC<UserInsightsHubProps> = ({ profile, isRtl }) => {
  const { t } = useTranslation();
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [insightsData, setInsightsData] = useState<any>(null);

  const analyzeUserInsights = async () => {
    setIsAnalyzing(true);
    try {
      // User context for AI
      const context = {
      ...
      const prompt = `Analyze this user's profile and generate a personalized "Smart Insights" dashboard.
      ...
      handleAiError(error, "User insights analysis");
      // Fallback data
      setInsightsData({
        status: 'good',
  ...
  useEffect(() => {
    const cachedData = smartCache.get(`insights_${profile.uid}`);
    if (cachedData) {
      setInsightsData({
        ...cachedData,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          >
            ...
            <div className="flex items-start justify-between mb-6 md:mb-8 relative z-10">
              <div>
                <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[10px] md:text-xs font-bold uppercase tracking-wider mb-3 md:mb-4 ${getStatusColor(insightsData.status)}`}>
                  <Activity size={14} />
                  {insightsData.status}
                </div>
                ...
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-brand-primary flex items-center justify-center mb-2 relative">
                  <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 100 100">
                    ...
                    <circle 
                      ...
                      strokeDasharray={`${insightsData.growthScore * 2.89} 289`}
                      strokeLinecap="round"
                    />
                  </svg>
                  <span className="text-xl md:text-2xl font-black text-brand-text-main">{insightsData.growthScore}</span>
                </div>
